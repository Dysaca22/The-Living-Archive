...
async function startServer() {
  ...
  function validateToken(res) {
    ...
    if (!token) {
      res.status(503).json({
        error: "TMDB no esta configurado en el servidor. Define TMDB_READ_ACCESS_TOKEN en .env y reinicia el servidor.",
      });
      return null;
    }
    return token;
  ...
  app.get("/api/tmdb/:mediaType/:id", async (req, res, next) => {
    ...
    try {
      ...
      if (!response.ok) {
        return res.status(response.status).json({ error: `Error de TMDB: ${response.status}` });
      }
      ...
      res.status(500).json({ error: "No se pudo obtener el detalle desde TMDB." });
    }
  });

  app.get("/api/tmdb/:mediaType/:id/reviews", async (req, res, next) => {
    const token = validateToken(res);
    if (!token) return;
    ...
    try {
      ...
      res.status(500).json({ error: "No se pudieron obtener reviews publicas de TMDB." });
    }
  });
  ...
  app.get("/api/tmdb/collection/by-id/:collectionId", async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: "No se pudieron obtener datos de la coleccion TMDB." });
    }
  });
  ...
  app.get("/api/tmdb/search", async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: "No se pudo consultar TMDB." });
    }
  });
  ...
  app.get("/api/tmdb/search/multi", async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: "No se pudo ejecutar la busqueda multiple en TMDB." });
    }
  });
  ...
  app.get("/api/tmdb/trending/:mediaType/:window", async (req, res) => {
    ...
    if (!["all", "movie", "tv"].includes(mediaType)) {
      return res.status(400).json({ error: "Tipo de medio invalido. Usa all, movie o tv." });
    }
    if (!["day", "week"].includes(window)) {
      return res.status(400).json({ error: "Ventana de tiempo invalida. Usa day o week." });
    }
    ...
    try {
      ...
      res.status(500).json({ error: "No se pudieron obtener tendencias de TMDB." });
    }
  });
  ...
  app.get("/api/tmdb/discover/:mediaType", async (req, res) => {
    ...
    if (!["movie", "tv"].includes(mediaType)) {
      return res.status(400).json({ error: "Tipo de medio invalido. Usa movie o tv." });
    }
    ...
    try {
      ...
      res.status(500).json({ error: "No se pudo descubrir contenido en TMDB." });
    }
  });
